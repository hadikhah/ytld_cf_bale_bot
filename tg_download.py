...
# ---------- Parallel download ----------
async def download_chunk(client, location, offset, size, part_num):
    """Download a single chunk and write it to a temporary file."""
...
async def download_parallel(client, msg, file_path, progress_callback, total):
    doc = msg.document
    ...
    print(f"[Download] Splitting into {len(chunks)} parts, {PARALLEL_CHUNKS} parallel")
    ...
    for batch_start in range(0, len(chunks), PARALLEL_CHUNKS):
        ...
        for i, (off, sz) in enumerate(batch):
            tasks.append(download_chunk(client, location, off, sz, batch_start + i))
        await asyncio.gather(*tasks)

        # Update progress
        progress_callback(sum(chunks[batch_start][1] for _ in batch), total)  # This isn't accurate, but we'll fix in final
...
async def main():
    ...
    try:
        ...
        def progress_callback(delta, total):
            nonlocal last_update
            progress_callback.downloaded += delta
            now = time.time()
        ...
        await download_parallel(client, message, download_path, progress_callback, message.document.size)
